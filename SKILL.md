...
reload, then paste `extract.js` into the DevTools console.
...
```sh
node ~/Dev/figma-make-export/unpack.mjs ~/Downloads/<slug>-export.json <target-dir>
```
...
```sh
node ~/Dev/figma-make-export/check-imports.mjs <target-dir>
```
